    ...
    <AppShell>
      <div className="container max-w-3xl py-10 md:py-16">
        <div className="fade-in text-center">
          <p className="section-label">{t("processing.eyebrow")}</p>
        ...
        <div className="mt-10 grid gap-8 md:grid-cols-[0.9fr_1.1fr]">
          ...
          <div className="order-2 md:order-1">
            ...
            <div className="card-glow mt-3 rounded-lg border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-4">
              <p className="text-xs text-[hsl(var(--muted))]">{t("processing.claimUnderAnalysis")}</p>
              <p className="mt-1 text-sm font-medium leading-snug text-[hsl(var(--foreground))]">
                {input.claim.event}
              </p>
              <p className="mt-1 text-xs text-[hsl(var(--muted))]">
                {[input.claim.location, input.claim.date].filter(Boolean).join(" · ") || t("processing.noContext")}
              </p>
            </div>
          </div>
