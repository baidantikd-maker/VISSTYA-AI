    ...
    <AppShell>
      ...
      <div className="container max-w-5xl py-10 md:py-14">
        <div className="fade-in text-center">
          <p className="section-label section-label-tone">{t("dashboard.overview")}</p>
          <h1 className={`mt-3 text-balance ${isDark ? "text-4xl font-bold md:text-6xl" : "text-3xl md:text-4xl"}`}>
            {t(`dashboard.greeting${greeting()}`)}, <span className="text-[hsl(var(--muted))]">{t("dashboard.researcher")}</span>
          </h1>
          <div className="mt-6 flex justify-center">
            <button
              ...
              className={
                isDark
                  ? "inline-flex min-h-11 items-center gap-2 rounded-md border border-white/50 bg-white/10 px-6 text-base font-bold text-[hsl(var(--foreground))] shadow-[0_0_10px_rgba(255,255,255,0.35),0_0_22px_rgba(255,255,255,0.15)] backdrop-blur-sm transition-all duration-200 hover:bg-white hover:text-black active:scale-[0.98]"
                  : "inline-flex min-h-11 items-center gap-2 rounded-md bg-[hsl(var(--primary))] px-5 text-sm font-medium text-[hsl(var(--primary-foreground))] transition-all duration-200 hover:opacity-90 active:scale-[0.98]"
              }
            >
        ...
        <div className="mt-8 grid grid-cols-2 gap-4 lg:grid-cols-4">
          <StatCard label={t("dashboard.totalVerifications")} value={stats.total} />
          <StatCard label={t("dashboard.trustable")} value={stats.trustable} className={isDark ? "stat-number-trustable" : "text-trustable"} />
          <StatCard label={t("dashboard.average")} value={stats.average} className={isDark ? "stat-number-average" : "text-average"} />
          <StatCard label={t("dashboard.false")} value={stats.false} className={isDark ? "stat-number-false" : "text-false"} />
        </div>

        {/* Recent */}
        <div className="mt-12">
          <div className="flex items-center justify-between">
            <h2 className={isDark ? "text-2xl font-semibold text-white md:text-3xl" : "text-xl md:text-2xl"}>{t("dashboard.recentVerifications")}</h2>
            <button
              type="button"
              onClick={() => setLocation("/history")}
              className={
                isDark
                  ? "inline-flex min-h-9 items-center gap-2 rounded-md bg-[hsl(261_88%_60%)] px-4 text-sm font-medium text-white transition-all duration-200 hover:bg-[hsl(261_88%_66%)] hover:shadow-[0_0_12px_hsl(261_88%_60%_/_0.6)] active:scale-[0.98]"
                  : "link-arrow"
              }
            >
              {t("dashboard.viewAll")}
