...
Extract any visible text or readable signage as an array of strings.
Flag any signs of AI generation, editing, or manipulation including inconsistent shadows, artifacts, warped text, unnatural faces or limbs, and visual seams.
Compare what you observe with the claimed event and claimed location, and report each as "consistent", "inconsistent", or "inconclusive". If a claim detail is not provided, set its consistency to null.
Always respond with a valid JSON object matching the schema exactly.`;
...
export async function analyzeMediaWithGemini(
  ...
  const key = assertApiKey();
  const { mediaUrl, mediaType, claimEvent, claimLocation, mediaBytes, mediaFilename } = params;
  ...
  if (mediaBytes) {
    ...
    if (!type?.mime) {
      throw new Error("Unable to determine MIME type from provided media bytes");
    }
    ...
    throw new Error("Either mediaUrl or mediaBytes must be provided for Gemini analysis");
  }
